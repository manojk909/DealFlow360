...
| AC-5 | Approved, then fulfilment splits across warehouses | **PASS** | 7/7 |
...
**44 checks, all passing. No failures, so no tasks were raised.**
...
- AC-6 proves one-time and recurring are billed through **separate artefacts**, which is
  ...
  correctly kept off the invoice; where it goes next is not built.
